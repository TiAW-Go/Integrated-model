...
class MolGraphDataset(data.Dataset):
    ...
    def __init__(self, path, prediction=False):
        with gzip.open(path, 'r') as file:
            self.header_cols = file.readline().decode('utf-8')[:-2].split(' ')#将一行数据按照一定符号进行分割
        n_cols = len(self.header_cols)
        print(n_cols)

        self.target_names = self.header_cols[2:]#设置表头
        ...
        if prediction:
            ...
            self.targets = np.genfromtxt(path, delimiter=' ', skip_header=1, usecols=range(1, n_cols), comments=None).reshape(-1, n_cols - 2)

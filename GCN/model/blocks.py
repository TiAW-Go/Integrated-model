...
def get_skip_connection(_X, X):
    # print("_X.getshape0:", _X.get_shape()[0], "X_getshape0:", X.get_shape()[0])
    # print("_X.getshape1:", _X.get_shape()[1], "X_getshape1:", X.get_shape()[1])
    # print("_X.getshape:", _X.get_shape()[2], "X_getshape:", X.get_shape()[2])
    if int(_X.get_shape()[2]) != int(X.get_shape()[2]):
        ...
        _X = tf.nn.relu(_X + X)
    print(_X.get_shape()[0], _X.get_shape()[1], _X.get_shape()[2])
    return _X
...
def graph_conv_gate(A, X, weight, bias, label):
    ...
    if int(X.get_shape()[2]) != dim:
        # print("X.shape[2]1:", int(X.get_shape()[2]))
        X = tf.layers.dense(X, dim, use_bias=False)
        # print(int(X.get_shape()[0]), ",", int(X.get_shape()[1]), ",", int(X.get_shape()[2]))
...
def graph_conv(A, X, weight, bias):
    dim = int(weight.get_shape()[1])
    num_atoms = int(A.get_shape()[1])
    # print("dim:", dim, "num_atoms:", num_atoms)
    _b = tf.reshape(tf.tile(bias, [num_atoms]), [num_atoms, dim])
    _X = tf.einsum('ijk,kl->ijl', X, weight) + _b
    _X = tf.matmul(A, _X)
...
def encoder_gat_gate(X, A, num_layers):
    ...
    for i in range(len(hidden_dim) - 1):
        ...
        for j in range(num_attn):
            conv_weight.append(
                tf.get_variable('ecw' + str(i) + '_' + str(j), initializer=tf.contrib.layers.xavier_initializer(),
                                shape=[hidden_dim[i], hidden_dim[i + 1]], dtype=tf.float64))
            conv_bias.append(
                tf.get_variable('ecb' + str(i) + '_' + str(j), initializer=tf.contrib.layers.xavier_initializer(),
                                shape=[hidden_dim[i + 1]], dtype=tf.float64))
            attn_weight.append(
                tf.get_variable('eaw' + str(i) + '_' + str(j), initializer=tf.contrib.layers.xavier_initializer(),
                                shape=[hidden_dim[i + 1], hidden_dim[i + 1]], dtype=tf.float64))
...
def encoder_gcn_gate(X, A, num_layers):
    ...
    gates = []
    for i in range(len(hidden_dim) - 1):
        conv_weight = tf.get_variable('ecw' + str(i), initializer=tf.contrib.layers.xavier_initializer(),
                                      shape=[hidden_dim[i], hidden_dim[i + 1]], dtype=tf.float64)
        conv_bias = tf.get_variable('ecb' + str(i), initializer=tf.contrib.layers.xavier_initializer(),
                                    shape=[hidden_dim[i + 1]], dtype=tf.float64)
...
def encoder_gcn(X, A, num_layers):
    ...
    num_atoms = int(X.get_shape()[1])
    input_dim = int(X.get_shape()[2])
    # print("num_atoms:", num_atoms, "input_dim:", input_dim)
    hidden_dim = []
    hidden_dim.append(input_dim)
    for i in range(num_layers):
        hidden_dim.append(32)
...
def readout_atomwise(X, latent_size):
    ...
    Z = tf.einsum('ijk,kl->ijl', X, weight['mlp_f0']) + tf.reshape(tf.tile(bias['mlp_f0'], [num_atoms]),
                                                                   [num_atoms, hidden_dim[0]])
    Z = tf.nn.relu(Z)
...
def readout_graph_gather(X0, X1, latent_size):
    ...
    weight = {
        'mlp_f0i': tf.get_variable("fw0i", initializer=tf.contrib.layers.xavier_initializer(),
                                   shape=[feature_size0 + feature_size1, hidden_dim[0]], dtype=tf.float64),
        'mlp_f0j': tf.get_variable("fw0j", initializer=tf.contrib.layers.xavier_initializer(),
                                   shape=[feature_size1, hidden_dim[0]], dtype=tf.float64),
        'mlp_f1': tf.get_variable("fw1", initializer=tf.contrib.layers.xavier_initializer(),
                                  shape=[hidden_dim[0], hidden_dim[1]], dtype=tf.float64),
        'mlp_f2': tf.get_variable("fw2", initializer=tf.contrib.layers.xavier_initializer(),
                                  shape=[hidden_dim[1], hidden_dim[2]], dtype=tf.float64),
    ...
    bias = {
        'mlp_f0i': tf.get_variable("fb0i", initializer=tf.contrib.layers.xavier_initializer(), shape=[hidden_dim[0]],
                                   dtype=tf.float64),
        'mlp_f0j': tf.get_variable("fb0j", initializer=tf.contrib.layers.xavier_initializer(), shape=[hidden_dim[0]],
                                   dtype=tf.float64),
        'mlp_f1': tf.get_variable("fb1", initializer=tf.contrib.layers.xavier_initializer(), shape=[hidden_dim[1]],
                                  dtype=tf.float64),
        'mlp_f2': tf.get_variable("fb2", initializer=tf.contrib.layers.xavier_initializer(), shape=[hidden_dim[2]],
    ...
    X_i = tf.nn.sigmoid(
        tf.einsum('ijk,kl->ijl', X_concat, weight['mlp_f0i']) + tf.reshape(tf.tile(bias['mlp_f0i'], [num_atoms]),
                                                                           [num_atoms, hidden_dim[0]]))
    X_j = tf.nn.relu(tf.einsum('ijk,kl->ijl', X1, weight['mlp_f0j']) + tf.reshape(tf.tile(bias['mlp_f0i'], [num_atoms]),
                                                                                  [num_atoms, hidden_dim[0]]))
